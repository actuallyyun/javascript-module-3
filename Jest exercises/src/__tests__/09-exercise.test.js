...
describe("09-exercises", () => {
  ...
  test("increment returns 10 for 7 + 3", () => {
    ...
    const result = addSpy(7, 3, calculator)
    ...
    //    numbers 7 and 3
    expect(addSpy.mock.calls[0].slice(0, 2)).toEqual([7, 3])
  });
  ...
  test("increment returns 15 for 10 + 5", () => {
     ...
     *    and store the result in the `result` variable
     */
    const result = addSpy(10, 5, calculator)
    ...
    expect(addSpy.mock.calls[0].slice(0, 2)).toEqual([10, 5])
  });
});
